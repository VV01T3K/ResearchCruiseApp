...
function EquipmentTable(props: EquipmentProps) {
  ...
  const mdColTitles = [
    "Lp.",
    "Nazwa sprzętu / aparatury",
    "Data zgłoszenia do ubezpieczenia (jeśli zgłoszono)",
    ...
    rules: {
      ...
      validate: {
        ...
        bothDatesNeeded: (value: ResearchEquipment[]) =>
          ...
          value.some(
            (row) =>
              (row.insuranceStartDate && !row.insuranceEndDate) ||
              (!row.insuranceStartDate && row.insuranceEndDate)
          )
            ? "Podaj obydwie daty"
            : true,
        rightYearPeriod: (value: FieldValues) => {
          if (
            value.some(
              (row: ResearchEquipment) =>
                row.insuranceStartDate &&
                row.insuranceEndDate &&
                row.insuranceEndDate < row.insuranceStartDate
            )
          ) {
