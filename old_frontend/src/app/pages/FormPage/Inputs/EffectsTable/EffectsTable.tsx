...
type TaskTableProps = FieldProps & { historicalTasks?: ResearchTaskEffect[] }
...
const TaskRowLabel = (row: ResearchTaskEffect) =>
    ...
    rules: {
      ...
      validate: {
        notEmptyArray: (value: FieldValues) => {
          if (
            value.some((row: ResearchTaskEffect) => {
              return Object.keys(taskTypesDefaultValues[Number(row.type)]).some((key) => {
                return key == "type"
                  ? false
                  : row[key as keyof ResearchTaskEffect] == "" ||
                      row[key as keyof ResearchTaskEffect] == undefined
              })
            })
          ) {
