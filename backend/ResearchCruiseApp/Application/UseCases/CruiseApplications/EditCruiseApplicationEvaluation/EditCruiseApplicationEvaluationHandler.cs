...
) : IRequestHandler<EditCruiseApplicationEvaluationCommand, Result>
{
    public async Task<Result> Handle(
        EditCruiseApplicationEvaluationCommand request,
        CancellationToken cancellationToken
    )
    {
        var cruiseApplication = await cruiseApplicationsRepository.GetByIdWithFormA(
            request.Id,
            cancellationToken
        );
    ...
    private async Task EditResearchTasksEvaluations(
        CruiseApplicationEvaluationsEditsDto cruiseApplicationEvaluationEditsDto,
        CancellationToken cancellationToken
    )
    {
        foreach (
            var researchTaskEvaluationEdit in cruiseApplicationEvaluationEditsDto.ResearchTasksEvaluationsEdits
        )
        {
            var formAResearchTask = await formAResearchTasksRepository.GetById(
                researchTaskEvaluationEdit.EvaluationId,
                cancellationToken
            );
    ...
    {
        foreach (
            var contractEvaluationEdit in cruiseApplicationEvaluationEditsDto.ContractsEvaluationsEdits
        )
        {
            var formAContract = await formAContractsRepository.GetById(
                contractEvaluationEdit.EvaluationId,
                cancellationToken
            );
    ...
    {
        cruiseApplication.FormA!.UgUnitsPoints =
            cruiseApplicationEvaluationEditsDto.NewUgUnitsPoints;
    }
    ...
    {
        foreach (
            var publicationEvaluationEdit in cruiseApplicationEvaluationEditsDto.PublicationsEvaluationsEdits
        )
        {
            var formAPublication = await formAPublicationsRepository.GetById(
                publicationEvaluationEdit.EvaluationId,
                cancellationToken
            );
    ...
    {
        foreach (
            var spubTaskEvaluationEdit in cruiseApplicationEvaluationEditsDto.SpubTaskEvaluationsEdits
        )
        {
            var formASpubTask = await formASpubTasksRepository.GetById(
                spubTaskEvaluationEdit.EvaluationId,
                cancellationToken
            );
