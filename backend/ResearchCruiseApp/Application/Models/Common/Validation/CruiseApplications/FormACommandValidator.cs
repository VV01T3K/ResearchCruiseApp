...
public class FormACommandValidator: AbstractValidator<FormACommand>
{
    ...
    private void AddShipUsageNonDraftValidation()
    {
        ...
        When(command => IsDifferentUsage(command.FormADto.ShipUsage), () =>
        {
        ...
        RuleForEach(command => command.FormADto.ResearchTasks)
            .Must(researchTaskDto =>
            {
                try
                {
                    ...
                    return type is
                        ResearchTaskType.BachelorThesis or
                        ResearchTaskType.MasterThesis or
                        ResearchTaskType.DoctoralThesis or 
                        ResearchTaskType.ProjectPreparation or
                        ...
                        ResearchTaskType.CommercialProject or
                        ResearchTaskType.Didactics or
                        ResearchTaskType.OwnResearchTask or
    ...
    private void AddResearchTasksNonDraftValidation()
    {
        ...
        RuleForEach(command => command.FormADto.ResearchTasks)
            ...
            .WithMessage("Wymagane jest podanie wszystkich szczegółów i poprawnego typu planowanych zadań.");
    }
    ...
    private void AddResearchTasksCommonValidation()
    {
        RuleForEach(command => command.FormADto.ResearchTasks)
            ...
            .WithMessage("Podana wartość liczbowa w szczegółach zadania jest w niepoprawnym formacie.");
    }
    ...
    private void AddContractsCommonValidation()
    {
        ...
        RuleForEach(command => command.FormADto.Contracts)
            .Must(contractDto =>
                contractDto.Scan is { Name: "", Content: "" } ||
                _fileInspector.IsFileSizeValid(contractDto.Scan.Content, FileConstants.MaxFileSize))
    ...
    private void AddUgTeamsDraftValidation()
    {
        RuleForEach(command => command.FormADto.UgTeams)
            ...
            .WithMessage("Liczebność uczestników z jednostki organizacyjnej UG podano w niepoprawnym formacie.");
    }
    ...
    private void AddUgTeamsNonDraftValidation()
    {
        RuleFor(command => command.FormADto.UgTeams)
            ...
            .WithMessage("Należy podać przynajmniej jeden zespół badawczy z jednostki organizacyjnej UG.");
        
        RuleFor(command => command.FormADto.UgTeams)
            .Must(ugTeams =>
                ugTeams
                    .DistinctBy(ugTeam => ugTeam.UgUnitId)
                    .Count() ==
                ugTeams.Count)
        ...
        RuleForEach(command => command.FormADto.UgTeams)
            .Must(ugTeamDto =>
                TryCountCrewMembers(ugTeamDto, out var count) &&
                count > 0)
            .WithMessage("Z każdej wybranej jednostki w rejsie musi uczestniczyć co najmniej jedna osoba.");
    }
    ...
    private void AddPublicationsCommonValidation()
    {
        ...
        RuleForEach(command => command.FormADto.Publications)
            .Must(publicationDto =>
                uint.TryParse(publicationDto.MinisterialPoints, out _))
            .WithMessage("Podano liczbę punktów ministerialnych publikacji w niepoprawnym formacie.");
    }
    ...
    {
        return shipUsage == FormAValuesConstants.ShipUsages
            .IndexOf(FormAValuesConstants.DifferentUsage)
            .ToString();
    }
    ...
    {
        if (uint.TryParse(ugTeamDto.NoOfEmployees, out var noOfEmployees) &&
