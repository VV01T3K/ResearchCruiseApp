...
{
    public UserEffectsRepository(ApplicationDbContext dbContext)
        : base(dbContext) { }

    public Task<List<UserEffect>> GetAllByUserIdWithCruiseApplication(
        Guid userId,
    ...
    {
        return DbContext
            .UserEffects.Where(userEffect => userEffect.UserId == userId)
