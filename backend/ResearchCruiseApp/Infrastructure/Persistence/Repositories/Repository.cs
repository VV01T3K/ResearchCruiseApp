    ...
    where T : Entity
{
    ...
    public Task<TUniqueEntity?> Get<TUniqueEntity>(
        TUniqueEntity searchedEntity,
    ...
    {
        var oldEntity = await DbContext
            .Set<T>()
            .FirstOrDefaultAsync(e => e.Id.Equals(newEntity.Id), cancellationToken);
        if (oldEntity != null)
        {
