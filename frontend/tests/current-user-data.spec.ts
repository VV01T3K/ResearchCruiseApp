...
test('current publications load from the v2 route', async ({ page }) => {
  ...
  let requested = false;
  await page.route(`${API_URL}/v2/users/me/publications`, async (route) => {
    requested = true;
    await route.fulfill({
      status: 200,
...
test('current publication import uses the v2 route', async ({ page }) => {
  ...
  await page.route(`${API_URL}/v2/users/me/publications`, (route) => {
    route.fulfill({
      status: 200,
      body: JSON.stringify([]),
      contentType: 'application/json',
  ...
  await page.route(`${API_URL}/v2/users/me/publications/import`, async (route) => {
...
test('current publications delete one and all through v2 routes', async ({ page }) => {
  ...
  await page.route(`${API_URL}/v2/users/me/publications/${publication.id}`, async (route) => {
    deletedOne = true;
    await route.fulfill({ status: 204 });
  });
...
test('current cruise effects load from the v2 route', async ({ page }) => {
  await seedAuthenticatedUser(page);
  let requested = false;
  await page.route(`${API_URL}/v2/users/me/cruise-effects`, async (route) => {
    requested = true;
    await route.fulfill({
      status: 200,
      body: JSON.stringify([]),
