        ...
        <div className="btn-group-vertical">
            {!user.accepted && (
                <div
                    ...
                    onClick={() =>
                        activateUser(user).then(() => updateUser('accepted', true))
                    }
                >
                    Aktywuj konto
                </div>
...
const activateUser = (user: UserData) =>
    Api.patch('/Users/unaccepted/' + user.id);
const deactivateUser = (user: UserData) =>
    Api.patch('/Users/' + user.id + '/deactivate');

const requestEmail = (user: UserData) =>
...
function ManageUsersPage() {
    ...
    const [withoutConfirmedMailToggle, setWithoutConfirmedMailToggle] =
        useState(false);
                ...
                <UsersContext.Provider value={userList}>
                    <FilteredUsersContext.Provider value={userList.filter(applyFilters)}>
                        <div className="form-page-content d-flex flex-column align-items-center w-100">
                            ...
                            <div
                                className={
                                    'd-flex flex-row flex-wrap justify-content-end w-100'
                                }
                            >
