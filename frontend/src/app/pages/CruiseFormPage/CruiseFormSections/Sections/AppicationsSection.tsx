...
const X = () => {
  const { applicationsAddingMode, EnableAddingModeButton, DisableAddingModeButton } =
  ...
  return (
    <>
      ...
      {remove && (
        <>
          <div className={"m-1"}>
            Do wszystkich kierowników zgłoszeń i ich zastępców zostanie wysłane powiadomienie o
            anulowaniu rejsu:
          </div>
          <div
            onClick={() => deleteCruise(cruiseId).then(() => navigate(Path.Cruises))}
            style={{ cursor: "pointer" }}
