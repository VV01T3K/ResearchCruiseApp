...
const CruiseManagersField = () => {
  ...
  useEffect(() => {
    const areFieldsEqual =
      mainManagerField && mainManagerField != EMPTY_GUID && mainManagerField == deputyManagerField
  ...
  useEffect(() => {
    if (
      cruiseUsers.map((user) => user.id).includes(cruise?.mainCruiseManagerId) &&
      ...
      if (!cruiseUsers.map((user) => user.id).includes(mainManagerField) && mainManagerField) {
        formContext?.setValue(cruiseManagerSectionFieldNames.mainCruiseManagerId, EMPTY_GUID)
      }
    }
    if (
      cruiseUsers.map((user) => user.id).includes(cruise?.mainDeputyManagerId) &&
      ...
      if (!cruiseUsers.map((user) => user.id).includes(deputyManagerField) && deputyManagerField) {
