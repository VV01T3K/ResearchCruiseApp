...
export const FieldTableWrapper = (
  ...
  const tableHeight = window.innerHeight - 270 < 600 ? window.innerHeight - 270 : 600
  const ColTitle = () => (
  ...
  const EmptyRow = () => (
    <>
      {reactWindow && (
        <div style={{ height: tableHeight }} className="table-field-no-content align-items-center">
          {emptyText}
        </div>
      )}
      {!reactWindow && <div className="table-field-no-content">{emptyText}</div>}
    </>
  )
  ...
  function cellWidth(colIndex: number) {
    return {
      width: windowWidth >= 768 ? `${colWidths[colIndex]}%` : "100%",
    }
  }

  const Render = (props: { className?: string }) => (
    <div className={"table-striped mh-100 d-flex flex-column " + props.className}>
      <TableHeader />
