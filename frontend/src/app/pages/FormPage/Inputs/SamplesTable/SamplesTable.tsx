...
function SamplesTable(props: SamplesProps) {
    ...
    const mdColTitles = ['Lp.', 'Rodzaj materiału badawczego / próbek / danych', 'Ilość / liczba', 'Analizy na zebranym materiale badawczym przeprowadzone podczas rejsu lub do przeprowadzenia po rejsie', 'Czy dane upubliczniono (jeśli tak, to w jaki sposób, czy przesłano je np. do BHMW)', ''];
    const colTitle = 'Rodzaje danych, próbek, materiału badawczego';
    const bottomMenu =
        <BottomMenuWithAddButton newOption={samplesDefault as SingleValue<any>} />;
    const emptyText = 'Nie dodano żadnego materiału';
    const { Render } = FieldTableWrapper(colTitle, mdColWidths, mdColTitles, samplesTableContent,
