...
const TaskTypeLabel = () => {
  const { rowValue } = CellTools()
  return <>{taskTypes[Number(rowValue.type)]}</>
}
  ...
  return (
    <KeyContext.Provider value={"newPoints"}>
      <div className={"task-field-input"}>
        <label className={"table-field-input-label"}>Przyznane punkty</label>
        <FIntField />
      </div>
    </KeyContext.Provider>
  )
...
export const pointsNotEmpty = <T extends Evaluation>(value: FieldValues) =>
  value.length <= 0 || !value.some((row: T) => row.evaluationId && row.newPoints == "")
