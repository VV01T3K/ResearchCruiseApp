...
const thesisOptions = thesisCategoriesPL.map((category, index) =>
    ({ label: category, value: thesisDefaultValues[index] }));


export const CategoryPicker = () => {
    const thesisCategoryOptions = thesisCategories.map(
        (category, index) => ({ label: thesisCategoriesPL[index], value: category }));
...
export const ThesesTable = (props: ThesesTableProps) => {
    ...
    const FilteredHistoricalTheses = (category: string) =>
        props.historicalTheses?.filter((row) => row.category == category)
            .map((row: Thesis) =>
                ({ label: ThesisRowLabel(row), value: row })) ?? [];

    const selectOptions = thesisCategories.map((thesisCategory, index) =>
        ({ label: thesisCategoriesPL[index], options: FilteredHistoricalTheses(thesisCategory) })) ?? [];
