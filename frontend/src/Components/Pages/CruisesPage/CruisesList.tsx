...
import {Number, Actions, Cruises, EndDate, MainCruiseManagerId, StartDate} from "./CruiseListFields";
...
export const CruisesContext = createContext<null | Cruise[]>(null)
...
export default function CruisesList() {
    ...
    const mdColTitles = ["Numer", "Czas rozpoczęcia", "Czas zakończenia", "Kierownik główny", "Zgłoszenia", "Akcje", ]
    const colTitle = "Rejsy"
    const emptyText = "Brak rejsów"
    const {Render} = FieldTableWrapper(colTitle, mdColWidths, mdColTitles,CruisesListTableContent,
        null, emptyText, cruisesToDisplay)
    ...
    return (
        <div className={"table-with-filters w-100"}>
            <div className={"d-flex flex-row w-100"}>
                ...
                <FilterMapper filterOptions={anyStringFilterOptions} optionClassName="d-flex col-3 p-1"/>
            </div>
