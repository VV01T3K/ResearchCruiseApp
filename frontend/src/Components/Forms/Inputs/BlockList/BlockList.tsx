...
function BlockList(props: Props) {
    ...
    const disabled= !Array
        .from({ length: data.length }, (_, index) => index)
        .some(item => (!fields.map((field) => field.label).includes(item)))
            ...
            <table className="table-striped w-100">
                ...
                <tbody>
                    ...
                    {fields.map((item, index) => (
                        <tr key={item.id}
                            className="d-flex flex-row justify-content-center align-items-center border bg-light"
                        >
                            <th className="w-25 text-center p-2 border-end ">{index}</th>
                            <th className="w-75 text-center p-2">{data[item.value]}</th>
                ...
                <Dropdown.Menu >
                    {data.map((item, index) => {
                        if (fields.map((field) => field.value).indexOf(index) < 0)
                            return(
                                <Dropdown.Item key={index} onClick={()=> { append({ value: index }) }}>
                                    {item}
                                </Dropdown.Item>
                            )
                    })}
