...
function TaskInput(props:{className:string, label:string, control, name:string, setValue}) {
    ...
    return (
        <div className={props.className + "  p-3"}>
            ...
            <div className="table-striped w-100">
                    ...
                    <div className={"d-flex flex-row center align-items-center"}>
                        ...
                        <div className={"d-none d-xl-inline-flex p-2"}>
                            <ButtonGroup as={Dropdown}
                                         className={"w-100 align-self-center" + Style.centeredDropdown}> {/* Dodaj klasę CSS dla dropleft */}
                ...
                <div className={"w-100"}>
                    {!fields && <div className={"d-flex flex-row justify-content-center  bg-light p-2 "}>
                        <div className={"text-center"}>Nie wybrano żadnego zadania</div>
                    </div>}
                    {fields && fields.map((item, index) => (
                        <div key={item.id}
                             className={"d-flex flex-wrap flex-row justify-content-center align-items-center border  bg-light "}>
                            <div
                                className={"text-center d-flex  col-12 col-xl-3 justify-content-center p-2"}>{options[item.type][0]}</div>
                            <div className={"text-center d-flex col-12 col-xl-8 justify-content-center"}>
                                {
                                    <div
                                        className={"d-flex p-2 flex-wrap"}>{Object.entries(item).slice(1, -1).map((t, s) => (
                                        <div className={"d-flex flex-row flex-wrap col-12 col-xl-6"}>
                                            <div className={"col-12 col-xl-3 "}>{t[0]}</div>
                                            <Controller
                                                name={`${props.name}[${index}].${t[0]}`}
                                                control={props.control}
                                                rules={{
                                                ...
                                                render={({field}) => (
                                                    <input {...field} type={"text"} className={"col-12 col-xl-9"}
                                                           onBlur={(e) => {
                                                               props.setValue(`${props.name}[${index}].${t[0]}`, e.target.value, {shouldDirty: false})
                                                               props.setValue(`${props.name}[${index}].${t[0]}`, e.target.value, {shouldValidate: true})
                                                           }}
                                                    ></input>)}/>
                            ...
                            <div
                                className={"d-inline-flex p-2   col-12  col-xl-1 justify-content-center justify-content-xl-end"}>
