...
function FormSection(props: {children?: React.ReactElement<any, string | React.JSXElementConstructor<HTMLElement>>[],
    ...
    return  (<div className="accordion-item border-2 border-black border-bottom">
                <div onClick={() => setIsActive(!isActive)} id={props.id} className={" accordion-title d-flex flex-row p-3  bg-light sticky-top "} >
        ...
        {isActive && <div className={"d-flex flex-row flex-wrap justify-content-center  pb-2"}>
                    {props.children}
                </div> }
            </div>
    )
