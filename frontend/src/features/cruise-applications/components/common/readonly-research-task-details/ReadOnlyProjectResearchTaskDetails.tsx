import { AppInput } from '@/components/inputs/AppInput';
import { AppNumberInput } from '@/components/inputs/AppNumberInput';
import { AppMonthPickerInput } from '@/components/inputs/dates/AppMonthPickerInput';
import { ProjectResearchTaskDto } from '@/features/cruise-applications/models/ResearchTaskDto';

type Props = {
  data: ProjectResearchTaskDto;
};
export function ReadOnlyProjectResearchTaskDetails({ data }: Props) {
  return (
    <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
      <AppInput
        name="researchTasks[].title"
        value={data.title}
        label="Tytuł"
        placeholder="Wprowadź tytuł"
        containerClassName="lg:col-span-2"
        disabled={true}
      />

      <AppMonthPickerInput
        name="researchTasks[].startDate"
        value={data.startDate}
        label="Data rozpoczęcia"
        disabled={true}
      />

      <AppMonthPickerInput
        name="researchTasks[].endDate"
        value={data.endDate}
        label="Data zakończenia"
        disabled={true}
      />

      <AppNumberInput
        name="researchTasks[].financingAmount"
        value={parseFloat(data.financingAmount)}
        type="float"
        minimum={0}
        label="Kwota finansowania [zł]"
        disabled={true}
      />

      <AppNumberInput
        name="researchTasks[].securedAmount"
        value={parseFloat(data.securedAmount)}
        type="float"
        minimum={0}
        label="Środki zabezpieczone na realizację rejsu [zł]"
        disabled={true}
      />
    </div>
  );
}
