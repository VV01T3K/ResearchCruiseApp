...
export const RefreshApplicationDetailsPage = () => {
  ...
  return () =>
    getCruiseApplication(location?.state.cruiseApplication.id).then(
      (response) =>
...
const AcceptApplication = (accept: string) => {
  ...
  const cruiseApplication = cruiseApplicationFromLocation();
  return () =>
    acceptApplication(cruiseApplication.id, accept).then(
      refreshApplicationDetailsPage
    );
};
...
export const MenuWithWarning = (props: {
...
export const CancelApplicationButton = () => {
  ...
  const ConfirmMenu = () => (
    ...
    >
      ...
      <div
        onClick={() =>
          acceptApplication().catch((err) => setError(err.request.response))
        }
    ...
    <>
      {(UserHasShipownerAccess() || UserHasAdminAccess()) && (
        <div className='form-page-option-bar'>
          ...
          {applicationContext!.status === CruiseApplicationStatus.Accepted && (
            <EditPointsMenu />
          )}
          {applicationContext!.status ===
          ...
          {applicationContext!.status === CruiseApplicationStatus.Reported && (
            <Reported />
          )}
        </div>
      )}
