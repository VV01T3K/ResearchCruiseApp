...
const SupervisorMenu = () => {
  ...
  const acceptPatch = (accept: string) =>
    acceptBySupervisor(cruiseApplicationId, supervisorCode, accept).catch((err) => {
...
export const BottomOptionBar = () => {
  ...
  const ReadonlyFormButtons = () => {
    ...
    const { UserHasGuestAccess, UserHasShipownerAccess, UserHasAdminAccess } = userBasedAccess()
    return (
      <>
        <PrintButton />
        ...
        {formContext?.type === FormType.B &&
          ...
          (cruiseApplication?.status === CruiseApplicationStatus.FormBFilled ||
            cruiseApplication?.status === CruiseApplicationStatus.Undertaken) && <RefillBButton />}
        {formContext?.type === FormType.C &&
          (UserHasAdminAccess() || UserHasShipownerAccess()) &&
          cruiseApplication?.status === CruiseApplicationStatus.Reported && <RefillCButton />}
