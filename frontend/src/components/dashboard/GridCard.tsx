...
export function GridCard({ name, className, iconClassName, Icon, description, href }: Props) {
    ...
    return (
      <div key={name} className={className}>
        <AppLink href={href} className="hover:no-underline">
          ...
          >
            <Icon className={cn('h-1/2', iconClassName)} strokeWidth={1.5} vectorEffect="non-scaling-stroke" />
  ...
  return (
    <div key={name} className={className}>
      <AppLink href={href} className="hover:no-underline">
        <motion.div
          className="relative flex h-full w-full flex-col justify-between overflow-hidden rounded-xl bg-white p-4 [box-shadow:0_0_0_1px_rgba(0,0,0,.03),0_2px_4px_rgba(0,0,0,.05),0_12px_24px_rgba(0,0,0,.05)]"
          onHoverStart={() => setIsHovered(true)}
          onHoverEnd={() => setIsHovered(false)}
        >
          <motion.div
          ...
          >
            ...
            >
              <Icon className={cn('size-full', iconClassName)} strokeWidth={1.5} vectorEffect="non-scaling-stroke" />
            </motion.div>
